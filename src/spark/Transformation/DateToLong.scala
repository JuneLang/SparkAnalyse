...
class DateToLong extends Transformer {
  override val uid: String = Identifiable.randomUID("dateToInt")
...
//  def setFormat(value: String): this.type = set(format, value)
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val seconds = udf { (time: java.sql.Timestamp) => {
      time.getTime
    }}
    dataset.withColumn($(outputCol), seconds(col($(inputCol))).as($(outputCol), outputSchema($(outputCol)).metadata))
  }
  ...
  override def transformSchema(schema: StructType): StructType = {
    ...
    val outCol: StructField = inputType match {
      case TimestampType =>
        StructField(outputColName, LongType)
